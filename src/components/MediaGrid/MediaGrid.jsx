		...
		.map((media) => {
			...
			if (media.media_type === 'movie' && movieGenres) {
				media.genres = media.genre_ids.map((id) => {
					const matchedGenre = movieGenres.find((genre) => genre.id === id);
					return matchedGenre?.name;
				});
			} else if (media.media_type === 'tv' && tvGenres) {
				media.genres = media.genre_ids.map((id) => {
					const matchedGenre = tvGenres.find((genre) => genre.id === id);
					return matchedGenre?.name;
				});
			}
			...
			<section className={styles.mediaGridContainer}>
				<header className={styles.header}>
					...
					<div className={styles.buttonContainer}>
						<Button className="gridButton" onClick={() => setCardStyle('Grid')}>
							<img className="icons" src={gridIcon} alt="Grid" />
						</Button>
						<Button className="listButton" onClick={() => setCardStyle('List')}>
				...
				<section className={`${styles.mediaGrid} ${styles['container' + cardStyle]}`}>
					{items
						.slice(0, limit)
						.map((media) =>
							cardStyle === 'Grid' ? (
								<Card
									...
									media={media}
									watchlist={watchlist}
									onCardClick={() => handleCardClick(media.id, media.media_type)}
