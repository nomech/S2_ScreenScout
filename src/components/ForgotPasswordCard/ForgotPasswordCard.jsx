...
const ForgotPasswordCard = () => {
	...
	const onSubmit = async (e) => {
		...
		try {
			...
			setSuccess(`A password reset link was sent to ${email.email} if it exists`);
			setEmail({ email: '' });
		} catch (err) {
			// If an error occurs, set the error message
			formatFireBaseFeedback(err.code, setError);
		} finally {
		...
		<form className={styles.forgotPasswordCard} onSubmit={(e) => onSubmit(e)}>
			...
			{!success && (
				<>
					...
					{error && <InfoCard style="error" text={error.firebase} />}
				</>
			)}
